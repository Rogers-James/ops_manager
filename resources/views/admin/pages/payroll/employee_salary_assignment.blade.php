...
@section('title', 'Admin - Employee Salary Assignment')

@section('main-content')

    <div class="container-xxl flex-grow-1 container-p-y">
        <div class="d-flex align-items-center justify-content-between mb-3">
            <h4 class="fw-bold mb-0"><span class="text-muted fw-light">Payroll Setup /</span> Employee Salary Assignment</h4>
            <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#assignSalary">
                    ...
                    <tbody class="table-border-bottom-0">
                        @forelse($assignments as $i => $a)
                            <tr>
                                ...
                                <td><strong>{{ $a->employee->employee_code ?? '' }}</strong> -
                                    {{ $a->employee->first_name ?? '' }} {{ $a->employee->last_name ?? '' }}</td>
                                <td>{{ $a->structure->name ?? '-' }}</td>
                                <td>{{ \Carbon\Carbon::parse($a->effective_from)->format('Y-m-d') }}</td>
                                <td>{{ $a->effective_to ? \Carbon\Carbon::parse($a->effective_to)->format('Y-m-d') : '-' }}
                                </td>
                                <td>{{ $a->notes ?? '-' }}</td>
                                <td class="d-flex gap-1">
                                    <button class="btn btn-sm btn-primary" data-bs-toggle="modal"
                                        data-bs-target="#editAssign-{{ $a->id }}"><i
                                            class="bx bx-edit-alt"></i></button>
                                    <form method="POST" action="{{ route('admin.employee_salary.destroy', $a) }}"
                            ...
                            <div class="modal fade" id="editAssign-{{ $a->id }}" data-bs-backdrop="static"
                                ...
                                <div class="modal-dialog modal-lg">
                                    <form class="modal-content" method="POST"
                                        action="{{ route('admin.employee_salary.update', $a) }}">
                                        @csrf @method('PUT')
                                                ...
                                                <div class="col-md-6 mb-2">
                                                    ...
                                                    <select name="salary_structure_id" class="form-select" required>
                                                        @foreach ($structures as $st)
                                                            <option value="{{ $st->id }}"
                                                                {{ (string) old('salary_structure_id', $a->salary_structure_id) === (string) $st->id ? 'selected' : '' }}>
                                                                {{ $st->name }}
                                                            </option>
                                                        @endforeach
                                                    </select>
    ...
    {{-- Assign Modal --}}
    <div class="modal fade" id="assignSalary" data-bs-backdrop="static" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <form class="modal-content" method="POST" action="{{ route('admin.employee_salary.store') }}">
                @csrf
                <input type="hidden" name="__modal" value="assignSalary">

                <div class="modal-header">
                    <h5 class="modal-title">Assign Salary Structure</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                ...
                <div class="modal-body">
                    <div class="row g-2">
                        ...
                        <div class="col-md-6 mb-2">
                            ...
                            <select name="salary_structure_id" class="form-select" required>
                                ...
                                @foreach ($structures as $st)
                                    <option value="{{ $st->id }}"
                                        {{ (string) old('salary_structure_id') === (string) $st->id ? 'selected' : '' }}>
                                        {{ $st->name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    @if ($errors->any() && old('__modal') === 'assignSalary')
                        <div class="alert alert-danger mt-2 mb-0">
                            <ul class="mb-0">
                                @foreach ($errors->all() as $e)
                                    <li>{{ $e }}</li>
